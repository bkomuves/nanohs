...
-- inlineProgram :: TopIdx -> Size -> Program Term -> Program Term
-- inlineProgram mainIdx sizeLimit blocks = 
--   termToProgram (removeUnusedLets mainIdx (inlineLets sizeLimit (programToTerm blocks)))
...
-- removeUnusedLets :: TopIdx -> Term -> Pair TopIdx Term
-- removeUnusedLets mainIdx term = 
--   let term' = removeUnusedLets' mainIdx term in Pair (findToplevelMain term') term'
...
-- removeUnusedLets :: Term -> Term
-- removeUnusedLets term = snd (go 0 setEmpty term) where
--   { goLev :: Level -> IntSet -> Name -> Level -> Pair IntSet (Named Var)
--   ; goLev level bound name j = Pair (ifte (setMember j bound) setEmpty (setSingleton j)) (Named name (LevV j)) 
--   ; goAtom :: Level -> IntSet -> Atom -> Pair IntSet Atom
--   ; goAtom level bound atom = case atom of { VarA nvar -> case nvar of { Named name var -> case var of
--       { LevV j   -> second VarA (goLev level bound name j)
--       ; IdxV i   -> second VarA (goLev level bound name (minus level (inc i)))
--       ; _        -> Pair setEmpty atom } } ; _ -> Pair setEmpty atom }
--   ; go :: Level -> IntSet -> Term -> Pair IntSet Term
--   ; go level bound term = debug "level" level (debug "bound" bound (debug "term" term (
--                           debug_ "term'" (case term of 
--       { AtmT atom      -> case goAtom level bound atom of { Pair set atom' -> Pair set (AtmT atom') }
--       ; LamT nt        -> case go (inc level) bound (forgetName nt) of { Pair set t' ->
--                           Pair (setDelete level set) (LamT (Named (nameOf nt) t')) }
--       ; AppT t a       -> case go     level bound t of { Pair set1 t' -> 
--                           case goAtom level bound a of { Pair set2 a' -> Pair (setUnion set1 set2) (AppT t' a') }}
--       ; PriT p as      -> let { pairs = map (goAtom level bound) as } in Pair (setUnions (map fst pairs)) (PriT p (map snd pairs)) 
--       ; LZPT p ts      -> let { pairs = map (go     level bound) ts } in Pair (setUnions (map fst pairs)) (LZPT p (map snd pairs)) 
--       ; LetT nt1 t2    -> case nt1 of { Named name t1 -> case go level bound t1 of { Pair free1 t1' ->
--                           case go (inc level) (setInsert level bound) t2 of { Pair free2 t2' -> ifte (setMember level free2)
--                             (Pair (setUnion free1 free2) (LetT (Named name t1') t2'))
--                             (Pair                 free2  (shiftTerm (inc level) level t2')) }}}
--       ; RecT n nts tm  -> let { level' = plus level n ; bound' = setInsertMany (rangeFrom level n) bound ;
--                                 pairs = map (\nt -> case nt of { Named named t -> case go level' bound' t of 
--                                   { Pair set t' -> Pair set (Named named t') }}) nts } 
--                           in  case go level' bound' tm of { Pair set tm' ->
--                                 Pair (setUnion set (setUnions (map fst pairs))) (RecT n (map snd pairs) tm') }
--       ; CasT la brs    -> case la of { Located loc a -> case goAtom level bound a of { Pair set1 a' -> 
--                           let { pairs = map (goBranch level bound) brs } 
--                           in  Pair (setUnion set1 (setUnions (map fst pairs))) (CasT (Located loc a') (map snd pairs)) }} 
--       ; MainT          -> Pair setEmpty MainT } ))))
--   ; goBranch :: Level -> IntSet -> BranchT -> Pair IntSet BranchT
--   ; goBranch level bound branch = case branch of
--       { DefaultT rhs    -> case go level bound rhs of { Pair set rhs' -> Pair set (DefaultT rhs') }
--       ; BranchT c n rhs -> let { level' = plus level n ; bound' = setInsertMany (rangeFrom level n) bound }
--                            in  case go level' bound' rhs of { Pair set rhs' -> Pair set (BranchT c n rhs') }} }
